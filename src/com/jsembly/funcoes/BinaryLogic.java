	...
	public static String resizeBinary(String binary, int newSize){
		...
		if(data.length() < newSize){
			if(binary.charAt(0) == '0'){
				while(data.length() < newSize){ data = '0' + data; }
			} else {
				while(data.length() < newSize){ data = '1' + data; }
			}
		}
		else if(data.length() > newSize) data = data.substring(data.length() - newSize, data.length());
		return data;
	}
	
	public static String and(String firstBinary, String secondBinary){
		String binary = new String();
		if(firstBinary.length() != 32) firstBinary = resizeBinary(firstBinary, 32);	
		if(secondBinary.length() != 32) secondBinary = resizeBinary(secondBinary, 32);	
